...
class Importer {
    ...
    #addListener() {
        ...
        this._dndAreaEl.addEventListener('new-canvas', async(e)=>{//キャンバス新規作成
    ...
    /*
    newCanvas(canvas, img) {
        const opt = {detail:{canvas: canvas ?? this._maker.make(img.width, img.height)}};
        if (img) {opt.detail.img = img}
        this._dndAreaEl.dispatchEvent(new CustomEvent('new-canvas', opt));
    }
    */
    async #loadFile(file) {
...
class PaintEditor {
    ...
    #init() {
        ...
        this._import = new Importer(this._el, this._maker, this._canvas, this._palette)
