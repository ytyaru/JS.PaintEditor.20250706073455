...
//        this._el.append(this._canvas.el, this._palette.el, this._export.el, this._maker.el, this._import.fileButton);
    ...
    #addListener() {
        ...
        this._el.addEventListener('new-canvas', async(e)=>{//キャンバス新規作成
            if (confirm('既存のキャンバスを破棄して宜しいですか？')) {
                this._canvas.el = e.detail.canvas; //canvas; // e.detail.el;
                if ('img' in e.detail) {this._canvas.ctx.drawImage(e.detail.img, 0, 0);}
                document.querySelector('canvas').replaceWith(e.detail.canvas);
            }
        });
        /*
        this._maker.el.addEventListener('new', async(e)=>{//キャンバス新規作成
            this._el.dispatchEvent(new CustomEvent('new-canvas', {detail:{
//                width: this._maker.width,
//                height: this._maker.height,
//                canvas: this._maker.make(e.detail.img.width, e.detail.img.height),
    ...
    /*
    #newCanvas(img) {
        this._maker.el.dispatchEvent(new CustomEvent('new', {detail:{
            width: this._maker.width,
            height: this._maker.height,
            canvas: this._maker.make(e.detail.img.width, e.detail.img.height),
            img: img,
